...
*/
    ...
    $("#searchbtn").click(function () {
        //TODO: COME UP WITH A VALIDATION TOCHECK TO SEE IF THIS city is in multiple locations...
        // validateLocation();
...
var req = $.jsonp({
       ...
       callbackParameter: "callback",
        ...
        success:function (data) {
            ...
            $("body").css({"cursor":""});
            $("#json").show();
            $("#json").append("Sorry this is the canadian search engine. For jobs in the US <a href='http://54.235.212.123/search?keyword="+$("#role").val()+"&local="+$("#location").val()+"'>click here:</a>");
            console.log("Error:"+err);
        },error:function(){
            console.log("Error is good this time returning false");
...
//$.sticky("We found " + data.TotalJobs + " jobs", {'speed' : 'fast',  'duplicates' : true, 'autoclose' : 5000 ,type: "st-success"});
    ...
    //We return false because we don't want a redirect..
        }//THis is the end of the first ajax call...
    });
}catch(err){
console.log("Error:"+err);
}
